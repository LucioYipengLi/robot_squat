...
class CommandsCfg:
    ...
    # 统一任务指令项（SquatWalkCommand）：6 维 [h_offset, vx, vy, ωz, pelvis_dx, pelvis_dy]，髋高偏移（相对
    ...
    # 观测经切片消费：速度 3 维、髋高 1 维，末尾追加骨盆偏置 2 维；不加入实际骨盆位置或启用标志。
    # 骨盆偏置仅 STAND/SQUAT 生效，双踝中点为零点，各 ±3 cm（骨盆口径不受上肢扰动污染，
    # 比旧 COM ±1 cm 放宽）；102 维观测不兼容旧 checkpoint。
    task_command = mdp.SquatWalkCommandCfg(
        ...
        ranges=mdp.SquatWalkCommandCfg.Ranges(
            ...
            lin_vel_y=(-0.15, 0.15),
            ang_vel_z=(-0.8, 0.8),
            pelvis_offset_x=(-0.03, 0.03),
            pelvis_offset_y=(-0.03, 0.03),
        ),
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        """102 维策略观测：保留原 100 维顺序，在末尾追加骨盆 X-Y 有效目标两维。

        原顺序为基座状态(9)、高度/速度指令(4)、全身关节状态(58)、上一步动作(12)、
        上肢 PD 目标(17)；骨盆偏置输入只含目标，不含实际骨盆位置、模式或启用标志。
        """
        ...
        pelvis_offset_cmd = ObsTerm(func=mdp.task_command_pelvis_xy, params={"command_name": "task_command"})
